...
module OpenID
  module Store
    class RailsCache < Interface
      attr_accessor :key_prefix

      def initialize(cache_client = Rails.cache, key_prefix='openid-store:')
      ...
      def store_association(server_url, association)
        ...
        [nil, association.handle].each do |handle|
          ...
          @cache_client.write(key, serialized, :expires_in => expiry(association.lifetime) )
        end
      end
      ...
      def use_nonce(server_url, timestamp, salt)
        ...
        return false if @cache_client.exist?(nonce_key)
        @cache_client.write(nonce_key, '', :expires_in => expiry(Nonce.skew + 5))
        return true
      end
